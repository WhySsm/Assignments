...
class game(arcade.Window):
    def __init__(self):
        super().__init__(width=1280,height=1000,title="pong game")
    ...
    def on_update(self,delta_time):#logical shoulde set here
        self.racket2.move()
        self.racket1.move()
         #ball hit upper_wall for racket reaction
        ...
        #ball hit lower_wall  for racket reaction
        if(int(self.ball.center_y)<self.ball.r):
            self.hit_upper_wall=False
            self.hit_lower_wall=True
              

        #racket1 how to hit the ball
        ...
        #ball hit upper_wall by racket for ball reaction
        if(int(self.ball.center_y)>self.ball.screen_height*2-20):
            self.hit_upper_wall=True
            self.hit_lower_wall=False
            if(self.ball.hitter==1):
                ...
                self.ball.change_x =  -1
                self.ball.change_y = -1
        #ball hit lower_wall by racket for ball reaction
        if(int(self.ball.center_y)<self.ball.r):
            self.hit_upper_wall=False
            self.hit_lower_wall=True
            if(self.ball.hitter==1):
    ...
    def on_draw(self):
        ...
        arcade.draw_text(score_text,self.width/50,self.height-100,arcade.color.BLACK,25,
                         font_name=(
        ...
        arcade.draw_text(score_text,18*self.width/20,self.height-100,arcade.color.BLACK,25,
                         font_name=(
                             "Times New Roman",  # Comes with Windows
                             "Times",  # MacOS may sometimes have this variant
                             "Liberation Serif"  # Common on Linux systems
